...
回调函数第一次执行时，previousValue 和 currentValue 可以是一个值，如果 initialValue 
在调用 reduce 时被提供，
那么第一个 previousValue 等于 initialValue ，并且currentValue 等于数组中的第一个值；
...
```js
 ...
 以便在JavaScript中实现“ 继承”的效果。  
```
```js
push和join方法想必大家并不陌生吧，JS数组中push()用于向数组末尾添加元素， 
JS数组中join() 用于将数组分割成字符串
```
```js
定义和用法
